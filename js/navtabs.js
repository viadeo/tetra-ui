            ...
            events: {
                model: {
                    navtabs: {
                        append: function(col) {
                            var obj = col[0];
                            me.currentContent = obj.get("html"), app.notify("end loading", {
                        ...
                        error: function(error) {
                            me.tabRef.pop(), app.notify("end loading", {
                                targetId: error.obj.get("targetId")
                            }).notify("show error").notify("set content", {
                                targetId: error.obj.get("targetId"),
                                content: me.currentContent,
                                tabRef: me.tabRef[me.tabRef.length - 1]
                            });
                ...
                view: {
                    ...
                    "show tab": function(data) {
                        me.tabRef.push(data.url), app.notify("start loading", {
                ...
                user: {
                    click: {
                        ".nav a[data-href]": function(e, elm) {
                            ...
                            me.methods.setActiveTab(elm), app.notify("show tab", data);
                        },
                        ".nav a[data-section]": function(e, elm) {
                            var targetId = elm.parents(".nav").attr("data-target-id");
                            _("#" + targetId).find(".section").removeClass("active"), _("#" + elm.attr("data-section")).addClass("active"), 
